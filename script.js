...
function changePoke() {
	...
	if (window.matchMedia("(max-width: 675px)").matches) {
		...
		if (poke.classList == 'pokeElement close') {

			poke.src = './images/pokemobile.png';
			poke.classList = 'pokeElement open';
			...
			poke.classList = 'pokeElement close';
...
async function prevPokemon() {
	...
	const atualPoke = await fetchPokemon(input.value.toLowerCase());
